...
public class Locators {
	...
	public static void main(String[] args) {
		...
		///////////////// SIGN IN WITH INCORRECT CREDS ///////////////////
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5));// wait for something to shoe... It is usually
																			// used when you are sure the element may be
																			// visible in a certain time, It is
																			// applicable to only a certain element
																			// which is specific to a certain condition.
		...
		driver.findElement(By.className("signInBtn")).click(); // if the class is like this :"submit signInBtn ", just
																// use the last name of the class.
		System.out.println(driver.findElement(By.cssSelector("p.error")).getText()); // css with tag.class extracts the
																						// text from the DOM element
		...
		driver.findElement(By.xpath("//input[@type='text'][2]")).clear();// Another form of XPATH selector using [index]
																			// and clear the input
		driver.findElement(By.cssSelector("input[type=\"text\"]:nth-child(3)")).sendKeys("john@gmail.com");// CSS
																											// selector
																											// with
																											// childs ,
																											// java
																											...
																											// \\ around
																											// text.
		driver.findElement(By.xpath("//form/input[3]")).sendKeys("981561561415");// XPath finding with parent to child
																					// traverse
		driver.findElement(By.cssSelector(".reset-pwd-btn")).click(); // css selector with .class
		String pass = (driver.findElement(By.cssSelector("form p")).getText()); // Css selector with parent to child
																				// traverse technique
		System.out.println("***Message for temporary password: " + pass);
		...
		driver.findElement(By.cssSelector("input[placeholder*='Pass']")).sendKeys("rahulshettyacademy"); // css selector
																											// with
																											// Regular
																											// Expressions
		try {
